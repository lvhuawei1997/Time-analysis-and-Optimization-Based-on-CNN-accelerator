...
from base import BaseProfiler, TimeMeasure
...
class FlopsProfiler(BaseProfiler):
    ...
    def profile(self, layer):
        ...
        time += self._estimate_remote_fetch(layer)
        return time
    ...
    def _profile_conv2d_gemm(self, layer, additional_mem=False):
        ...
        global filter_nonzero, input_nonzero
        flops = 2 * layer.filters[0] * layer.filters[1] * layer.filters[2] * (1.0 - ((1.0 - input_nonzero) / layer.strides[0]))
        ...
        flops *= filter_nonzero

        if input_nonzero <= 1:
            # Move every element in the input tensor.
            flops += 2 * np.prod(layer.inputs) * input_nonzero
        ...
        # print(input_nonzero, filter_nonzero)
        # print('holes filter: %.2f''  holes inputs: %.2f' % (layer.percent_holes_in_filters, layer.percent_holes_in_inputs))
        # print('GEMM flops: %d\n  holes filter: %.2f\n'
        #                    '  holes inputs: %.2f' %
        #                    (flops, layer.percent_holes_in_filters,
        #                     layer.percent_holes_in_inputs))
        ...
        input_size = (layer.inputs[0] * (layer.inputs[1] + 2 * layer._pad_h) *
                      (layer.inputs[2] + 2 * layer._pad_w) * layer.inputs[3]) * input_nonzero
        comm_time = self._estimate_comm_time(input_size * _BYTES_FLOAT)
        comm_time += self._estimate_comm_time(
            np.prod(layer.filters) * filter_nonzero *
            _BYTES_FLOAT)
        comm_time += self._estimate_comm_time(
